...
class CoreConfig(configparser.Config):
    ...
    def produce_fixed_observable(self, fixed_observable_input, theoryid, bsm_fac_data):
        if not bsm_fac_data:
            raise ConfigError("bsm_fac_data are required to fit fixed observables")
        try:
            return self.loader.check_fixed_observable(
                fixed_observable_input, theoryid, bsm_fac_data
            )
        except LoaderError as e:
            raise ConfigError(
    ...
    def produce_fixed_observables(
        self, fixed_observable_inputs=None, theoryid=None, bsm_fac_data=None
    ):
        if fixed_observable_inputs is None:
            fixed_observable_inputs = []
        return NSList(
            [
                self.produce_fixed_observable(f, theoryid, bsm_fac_data)
